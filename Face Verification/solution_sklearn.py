...
def perform_training(
    image_feature_list, image_index_list, description, feature_extension
):
    ...
    for fold_index, fold_item in enumerate(label_kfold):
        ...
        best_score = sklearn_related.train_model(
            X_train, Y_train, X_test, Y_test, model_path
        )
        best_score_array[fold_index] = best_score

        print(
            "For the {:d} fold, the sklearn model achieved the score {:.4f}.".format(
                fold_index + 1, best_score
            )
        )
...
def generate_prediction(
    ...
    model_path_rule = os.path.join(
        working_directory, "*" + common.SCIKIT_LEARN_EXTENSION
    )
    metric_list = METRIC_LIST_DICT[feature_extension]
...
def make_prediction(facial_image_extension, feature_extension):
    ...
    # Generate prediction
    prediction_file_prefix = (
        "Aurora_" + selected_facial_image + "_" + selected_feature + "_sklearn_"
    )
    generate_prediction(
        description,
        testing_file_content,
